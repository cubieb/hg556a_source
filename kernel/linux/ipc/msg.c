...
asmlinkage long kernel_sys_msgsnd (int msqid, struct msgbuf __user *msgp, size_t msgsz, int msgflg)
{
	struct msg_queue *msq;
	struct msg_msg *msg;
	long mtype;
	...
	msg = kernel_load_msg(msgp->mtext, msgsz);
	if(IS_ERR(msg))
		return PTR_ERR(msg);
...
EXPORT_SYMBOL(kernel_sys_msgsnd);
#endif
