...
/* Start of NP by h00101845 2008-06-14 增加wan侧组播日志记录 */
...
#define VALID_ADDR(x)\
	    ((ntohl(x) != INADDR_ALLRTRS_GROUP) && (ntohl(x) != INADDR_ALLRTRS_IGMPV3_GROUP) &&    \
	    (ntohl(x) != INADDR_ALLHOSTS_GROUP) && (ntohl(x) != INADDR_DVMPR) && \
	    (ntohl(x) != INADDR_PIM))

#define IS_UPNP(x)    (ntohl(x) != INADDR_UPNP)
/* End of NP by h00101845 2008-06-14 增加wan侧组播日志记录 */
...
extern int ip_mc_log_switch(struct sock *sk , int *log_switch);
