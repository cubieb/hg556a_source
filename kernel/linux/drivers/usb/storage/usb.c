 ...
 *
 * $Id: usb.c,v 1.2.2.1 2009/02/24 12:47:11 l43571 Exp $
 *
...
SkipForAbort:
...
#if 1
		/*增加数据卡PC切换模式l65130 2008-09-20 start*/
		if (g_iDataCardIn == 3)
		{
			us->iobuf[0] = 0x1;
		...
		/*增加数据卡PC切换模式l65130 2008-09-20 end*/
#endif
...
int Is_HuaweiVID(unsigned long us)
{

    struct us_data* usb_data = (struct us_data*)us;
    if (usb_data && (usb_data->pusb_dev))
    {
        return ((usb_data->pusb_dev->descriptor.idVendor == 0x12d1) ? 1 : 0);
    }
    
    return 0;
}
EXPORT_SYMBOL(Is_HuaweiVID);

/* <tanyin 2009.2.13>*/
char* get_vendor(unsigned long us)
{
    struct us_data* usb_data = (struct us_data*)us;
    if (usb_data && (usb_data->vendor))
    {
        return (usb_data->vendor);
    }
    
    return NULL;
}
EXPORT_SYMBOL(get_vendor);
