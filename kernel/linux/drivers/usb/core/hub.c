...
void usb_disconnect(struct usb_device **pdev)
{
	...
	dev_info (&udev->dev, "USB disconnect, address %d\n", udev->devnum);
   {
      ...
      /*判断HSPA是否已经拔出*/      
      if (udev->descriptor.idVendor == 0x12d1)
      {
...
#define HUB_DEBOUNCE_STABLE	  300
...
static void hub_events(void)
{
	...
	while (1) {
		...
		usb_get_dev(hdev);
		spin_unlock_irq(&hub_event_lock);
             /*BEGIN:modify for usb recognition problem:DTS2011011901689 by DHS00169988; 2011-3-7*/
	       msleep(800);
            /*END;by DHS00169988 */
...
int usb_reset_device(struct usb_device *udev)
{
	...
	while (down_trylock(&(udev)->serialize) != 0) {
