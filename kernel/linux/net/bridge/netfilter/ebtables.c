...
static inline int ebt_dev_check(char *entry, const struct net_device *device)
{
    ...
    {
	char *prefix_end = strchr(entry, '+');
	if('!' == entry[0])
    {
        if(prefix_end)
            return !!!strncmp(&entry[1], device->name, prefix_end - entry -1);
        else
            return !!!strcmp(&entry[1], device->name);
    }
	else
    {
        if(prefix_end)
            return !!strncmp(entry, device->name, prefix_end - entry);
        else
   ...
   struct ebt_table *table)
{
	...
	{
        // 协议类型默认为IP
        memcpy(&eth8021q, (**pskb).mac.ethernet, sizeof(struct ethhdr));
        if ((*pskb)->protocol == __constant_htons(ETH_P_8021Q))
        {
            ...
            {
                p8021q += (2 + 2); 
                eth8021q.h_proto = *(unsigned short*)p8021q; // VLAN+VALN+IP
                if ((*(unsigned short*)p8021q  == __constant_htons(ETH_P_PPP_SES)))
                {
                    if (*(unsigned short*)(p8021q + 2 + 6) == __constant_htons(0x0021))
                    {
                        eth8021q.h_proto = __constant_htons(ETH_P_IP); // VLAN+VLAN+PPP+IP
                    }
                }
            }
    #endif //CONFIG_KPN_VLAN
...
// #ifdef CONFIG_KPN_VLAN
		    && (ebt_basic_match(point, &eth8021q, in, out))//support QinQ by c47036
// #endif //CONFIG_KPN_VLAN
		    ) 
			goto letscontinue;
