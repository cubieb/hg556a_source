...
int __skb_linearize(struct sk_buff *skb, int gfp_mask)
{
     ...
     */
/*MODIFY BY YINHENG FOR WMM FEATUER OF 3.12 WIFI DEIVER*/
...
int dev_queue_xmit(struct sk_buff *skb)
{
	...
	struct Qdisc *q;
	int rc = -ENOMEM;
/*YeHong(42815) 2007/9/7 Add for BT QOS 问题 Begin*/
	struct net_device *dev_bak = 0, *dev_qos = 0;
/*YeHong(42815) 2007/9/7 Add  End */


    #if 0
    /*YeHong(42815) 2007/9/7 Add for BT QOS 问题 Begin*/
    dev1 = dev_get_by_name(skb->dev->name);
    if(dev1 == NULL)
       goto out_kfree_skb;
    /*YeHong(42815) 2007/9/7 Add  End */
    #endif
	...
	/*HG586V100R001 c40163 2007-8-27 BT add begin */
	dev_bak = dev;
	if (!(skb->nfmark & FROM_WAN)  && !(skb->nfmark & 0x80000000)  
	&& ((dev->name[0] != 'a') && (dev->name[0] != 'e'))
	&& !((dev->name[0] == 'b') && (dev->name[1] == 'r') && (dev->name[2] == '0')) 

	/*w00135358 add for HG556a  20090311-- WIFI接口为wifi0的流量不进入br9*/
/* BEGIN: Modified by aizhanlong 00169427, 2011/10/29   PN:HSPA upload speed lower than PC*/
	&& (dev->name[0] != 'w')&&(dev->name[8] != '6')) {
/* END:   Added by aizhanlong 00169427, 2011/10/29 */
	    skb->nfmark |= 0x80000000;
		dev_qos = dev_get_by_name("br9");
		if (dev_qos != 0) {
			dev = dev_qos;
		}
	}
	/*HG586V100R001 c40163 2007-8-27 BT add end */
