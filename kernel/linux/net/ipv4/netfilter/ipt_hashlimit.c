 ...
 *
 * $Id: ipt_hashlimit.c,v 1.1 2008/08/26 06:14:52 l65130 Exp $
 *
...
static spinlock_t hashlimit_lock;	/* protects htables list */
static spinlock_t hlimit_lock;	/* additional checkentry protection */
static HLIST_HEAD(hashlimit_htables);
static struct kmem_cache_t *hashlimit_cachep ;
...
//	if (hashlimit_init_dst(hinfo, &dst, skb, protoff) < 0)
	if (hashlimit_init_dst(hinfo, &dst, skb, 0) < 0)
		goto hotdrop;
           ...
           unsigned int hook_mask)
{
	...
	spin_lock_bh(&hlimit_lock);
	//r->hinfo = htable_find_get(r->name, match->family);
//	if (!r->hinfo && htable_create(r, match->family) != 0) {
	r->hinfo = htable_find_get(r->name, AF_INET);
	if (!r->hinfo && htable_create(r, AF_INET) != 0) {
		spin_unlock_bh(&hlimit_lock);
		return 0;
	}
	spin_unlock_bh(&hlimit_lock);
...
static void
hashlimit_destroy(/*const struct xt_match *match,*/void *matchinfo, unsigned int matchinfosize)
{
	struct xt_hashlimit_info *r = matchinfo;
...
static struct ipt_match xt_hashlimit = {
		.name		= "hashlimit",
//		.family		= AF_INET,
...
static int __init xt_hashlimit_init(void)
{
	...
	spin_lock_init(&hashlimit_lock);
	spin_lock_init(&hlimit_lock);

	err = ipt_register_match(&xt_hashlimit);
	if (err < 0)
		goto err1;
...
err2:
	ipt_unregister_match(&xt_hashlimit);
err1:
	return err;
