...
/*MODIFY BY YINHENG FOR 3.12 WIFIF DRIVER*/
    ...
    if(info->flag & IPTMAC_SRC)
        src_match = ((memcmp(skb->mac.ethernet->h_source, info->srcaddr, ETH_ALEN) == 0) ^ ((info->invert & IPTMAC_SRC) > 0));
    
    if(info->flag & IPTMAC_DST)
        dst_match = ((memcmp(skb->mac.ethernet->h_dest, info->dstaddr, ETH_ALEN) == 0) ^ ((info->invert & IPTMAC_DST) > 0));
