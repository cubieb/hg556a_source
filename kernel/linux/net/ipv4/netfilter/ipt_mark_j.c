...
#include <linux/netfilter_ipv4/ipt_mark_j.h>
       ...
       void *userinfo)
{
	...
	unsigned long ulSetMark, ulNewMark;
	const struct ipt_mark_target_info *markinfo = targinfo;
	
	ulSetMark = (markinfo->mark & markinfo->mask);
	ulNewMark = (((*pskb)->nfmark) & ~(markinfo->mask));
	ulNewMark |= ulSetMark;

	if((*pskb)->nfmark != ulNewMark) {
		(*pskb)->nfmark = ulNewMark;
