 ...
 *
 * ipt_ecn_j.c,v 1.5 2002/08/18 19:36:51 laforge Exp
*/
...
static inline int
set_ect_ip(struct sk_buff **pskb, const struct ipt_ecn_j_info *einfo)
{
...
/* Return 0 if there was an error. */
static inline int
set_ect_tcp(struct sk_buff **pskb, const struct ipt_ecn_j_info *einfo, int inward)
{
	struct tcphdr tcph;
	u_int16_t diffs[2];
...
target(struct sk_buff **pskb,
       ...
       void *userinfo)
{
	const struct ipt_ecn_j_info *einfo = targinfo;
...
{
	const struct ipt_ecn_j_info *einfo = (struct ipt_ecn_j_info *)targinfo;

	if (targinfosize != IPT_ALIGN(sizeof(struct ipt_ecn_j_info))) {
