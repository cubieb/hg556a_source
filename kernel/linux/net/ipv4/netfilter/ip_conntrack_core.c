 ...
 *  6 May 2006: caiyanfeng <caiyanfeng@huawei.com>
 *  - Fix the process of expectation deletion.
 * */
...
static void __unexpect_related(struct ip_conntrack_expect *expect)
{
	...
	/* Netfilter 3.1.3 porting by c47036 20060505 */
	if (expect->expectant != NULL)
	{
...
{
	/* Start of netfilter 3.1.3 porting by c47036 20060505 */
	    ...
	    && !del_timer(&expect->timeout))
	{
	    list_del(&expect->expected_list);
	    expect->expectant->expecting--;
	    expect->expectant = NULL;
	}
	else
	{
	    __unexpect_related(expect);
	}
	/* End of netfilter 3.1.3 porting by c47036 20060505 */
}
...
static void remove_expectations(struct ip_conntrack *ct, int drop_refcount)
{
	...
	list_for_each_safe(exp_entry, next, &ct->sibling_list) {
		...
		if (exp->sibling) {
			...
			/* Netfilter 3.1.3 porting by c47036 20060505 */
			if (drop_refcount && (exp->expectant != NULL)) {
				/* Indicate that this expectations parent is dead */
				list_del(&exp->expected_list);
		  ...
		  enum ip_conntrack_info *ctinfo)
{
    ...
    /* Start of conntrack cleanup by c47036 20060721 */
    if (ip_conntrack_dns)
    {
        ip_conntrack_user_cleanup();
    }
    /* Start of conntrack cleanup by c47036 20060721 */
				...
				struct ip_conntrack *related_to)
{
	...
	if (old) {
        ...
        /* Start of netfilter H.323 ALG by c47036 20060615 */
        /* Some helper registerd with null name, so checking is necessary. --c47036 */
        if ((old->expectant != NULL) 
            && (old->expectant->helper != NULL)
            && (old->expectant->helper->name != NULL)
            && (!strcmp(old->expectant->helper->name, "Q.931")
            || !strcmp(old->expectant->helper->name, "H.245")
            || !strcmp(old->expectant->helper->name, "RAS")))
        {
            unexpect_related(old);
            goto out;
        }
        /* End of netfilter H.323 ALG by c47036 20060615 */        
...
static int kill_dns(const struct ip_conntrack *i, void *data)
{
	return (i->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.u.udp.port == 53);
}
static int kill_sip(const struct ip_conntrack *i, void *data)
{
	unsigned short sipport = i->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.u.udp.port;
	return ((sipport == 5060) || ((sipport >= 7070) && (sipport <= 7079)));
}
static int kill_dport(const struct ip_conntrack *i, void *data)
{
	return (i->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.u.udp.port == *(int*)data);
}
void ip_conntrack_user_cleanup(void)
{
    switch (ip_conntrack_dns)
    {
        case 0: 
            // do nothing
            break;
        case 1: 
            // clean dns conntrack
            ip_ct_selective_cleanup(kill_dns, NULL);
            break;
        case 2: 
            // clean sip conntrack
            ip_ct_selective_cleanup(kill_sip, NULL);
            break;
        case 9: 
            // clean all for debug
            ip_ct_selective_cleanup(kill_all, NULL);
        ...
        default: 
            // clean all for debug
            ip_ct_selective_cleanup(kill_dport, &ip_conntrack_dns);
            break;            
    }
    ip_conntrack_dns = 0;
}
