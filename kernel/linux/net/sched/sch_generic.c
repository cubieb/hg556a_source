...
int qdisc_restart(struct net_device *dev)
{
	...
	if ((skb = q->dequeue(q)) != NULL) {
		...
		struct net_device *dev_bak = dev;
		if (dev->name[0] == 'b' && dev->name[1] == 'r' && dev->name[2] == '9') 
        {
			/*printk("qdisc_restart, dev=br9, skb_dev=%s\n", skb->dev);*/
		...
		if (spin_trylock(&dev->xmit_lock)) {
			...
			spin_unlock(&dev->xmit_lock);
			spin_lock(&dev->queue_lock);
			/* HG586V100R001 c40163 2007-8-27 BT modify begin
			q = dev->qdisc;
			*/
			
		    q = dev_bak->qdisc;
			/* HG586V100R001 c40163 2007-8-27 BT modify end */
		} else {
