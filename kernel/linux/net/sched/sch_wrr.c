...
static int mac_classify(struct mac_head* head, struct sk_buff *skb)
{
  ...
  if (head->usefw) {
	...
	unsigned int uMark = (skb->nfmark & 0xff);
#else
...
static int wrr_init(struct Qdisc *sch, struct rtattr *opt)
{
  ...
  printk("wrr_init, sch->handle=0x%x\n", sch->handle);
...
{
  struct wrr_sched_data *q = qdisc_priv(sch);
  //printk("wrr_leaf, arg=0x%x, q->bandc=0x%x\n", arg, q->bandc);
...
static unsigned long wrr_get(struct Qdisc *sch, u32 classid)
{
  ...
  //printk("wrr_get, classid=0x%x, band=0x%x, q->bandc=0x%x\n", classid, band, q->bandc);
                          ...
                          struct sk_buff *skb, struct tcmsg *tcm)
{
  ...
  //printk("wrr_dump_class, sch->handle=0x%x, cl=0x%x\n", sch->handle, cl);
                      ...
                      struct rtattr **tca, unsigned long *arg)
{
  ...
  printk("wrr_change, cl=0x%x, q->bandc=0x%x\n", cl, q->bandc);
