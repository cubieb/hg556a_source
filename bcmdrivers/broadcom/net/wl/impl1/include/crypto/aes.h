 ...
 *
 * $Id: aes.h,v 1.1 2008/08/25 06:41:17 l65130 Exp $
 */
...
#define	AES_CCMP_FC_MASK 		~(AES_CCMP_SUBTYPE_LOW_MASK | AES_CCMP_FC_RETRY | AES_CCMP_FC_PM | AES_CCMP_FC_MOREDATA)
...
int aes_ccmp_encrypt(unsigned int *rk, const size_t key_len,
	const size_t data_len, uint8 *p, bool legacy);
...
int aes_ccm_mac(unsigned int *rk,const size_t key_len,const uint8 *nonce,
	const size_t aad_len,const uint8 *aad,const size_t data_len, const uint8 *ptxt, uint8 *mac)	;
int aes_ccm_encrypt(unsigned int *rk, const size_t key_len, const uint8 *nonce, const size_t aad_len,
	const uint8 *aad, const size_t data_len, const uint8 *ptxt, uint8 *ctxt, uint8 *mac);
int aes_ccm_decrypt(unsigned int *rk,const size_t key_len,const uint8 *nonce, const size_t aad_len,
	const uint8 *aad, const size_t data_len, const uint8 *ctxt, uint8 *ptxt);
