 ...
 *
 * $Id: wlioctl.h,v 1.1 2008/08/25 06:41:17 l65130 Exp $
 */
...
typedef struct wl_aci_args {
	...
	int glitch_delay;	/* interval between ACI scans when glitch count is consistantly high */
} wl_aci_args_t;
...
typedef struct tx_power {
	uint8	txpwr_band_max[NUM_PWRCTRL_RATES];		/* User set target */
	uint8	txpwr_limit[NUM_PWRCTRL_RATES];		/* reg and local power limit for current channel (in .25 dBs) */
	uint8	txpwr_local_max;	/* local txpwr regulatory max according to our AP (in .25 dBs) */
	uint8	txpwr_local_constraint;	/* local power contraint according to our AP (in .25 dBs) */
	uint8	txpwr_chan_reg_max;	/* Regulatory max for this channel */
	uint8	txpwr_target[2][NUM_PWRCTRL_RATES];	/* Latest phy level pwr target for 2.4 and 5 Ghz bands */
	uint8	txpwr_est_Pout[2];	/* Latest pwr estimate for 2.4 and 5 Ghz bands */
	uint8	txpwr_opo[NUM_PWRCTRL_RATES];		/* On G phy, OFDM power offset */
	uint8	txpwr_bphy_cck_max[NUM_PWRCTRL_RATES];	/* Max CCK power for this band, from SROM */
	uint8	txpwr_bphy_ofdm_max;	/* Max OFDM power for this band, from SROM */
	uint8	txpwr_aphy_max[NUM_PWRCTRL_RATES];		/* Max power for A band, for current channel, from SROM */
