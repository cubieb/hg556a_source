 ...
 *
 * $Id: linux_osl.h,v 1.1 2008/08/25 06:42:07 l65130 Exp $
 */
...
#if defined(DSLCPE) && !defined(BCMSDIO)
	...
	SELECT_BUS_READ(osh, \
		({ \
            ...
            if ((uint32)r & 0x4) { \
                uint32 desc[3]; \
                uint32 dataIn; \
                *(uint32*)KSEG1ADDR(desc) = 0xD000 | (sizeof(*(r)) << 16); \
                *(uint32*)KSEG1ADDR(desc + 1) = CPHYSADDR(&dataIn); \
                *(uint32*)KSEG1ADDR(desc + 2) = CPHYSADDR(r); \
                MPI->locbuscntrl |= 0x20; \
                MPI->locch2descaddr = CPHYSADDR(desc); \
                MPI->locch2ctl = 0x101; \
                while ((*(volatile uint32*)KSEG1ADDR(desc)) & 0x8000); \
			    switch (sizeof(*(r))) { \
				    case sizeof(uint8):	__osl_v = \
					    readb((volatile uint8*)KSEG1ADDR((uintptr)(&dataIn) | 3)); break; \
				    case sizeof(uint16):	__osl_v = \
					    readw((volatile uint16*)KSEG1ADDR((uintptr)(&dataIn) | 2)); break; \
				    case sizeof(uint32):	__osl_v = \
					    readl((volatile uint32*)KSEG1ADDR(&dataIn)); break; \
                } \
            } else { \
		        switch (sizeof(*(r))) { \
			        case sizeof(uint8):	__osl_v = \
...
#define TICKDIFF(_x2_,_x1_)	((_x2_>=_x1_)?(_x2_-_x1_):((unsigned long)(-1)-_x2_+_x1_))
#else
...
#ifdef DSLCPE
#define	PKTPRIO(skb)			(((struct sk_buff*)(skb))->priority = ((struct sk_buff*)skb)->nfmark>>PRIO_LOC_NFMARK & 0x7)
#define	PKTSETPRIO(skb, x)		(((struct sk_buff*)(skb))->priority = (x), ((struct sk_buff*)skb)->nfmark &= ~(0xf << PRIO_LOC_NFMARK),  ((struct sk_buff*)skb)->nfmark |= (x & 0x7) << PRIO_LOC_NFMARK)
#else
