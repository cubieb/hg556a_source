 ...
 *
 * $Id: wlu.c,v 1.1 2008/08/25 06:41:49 l65130 Exp $
 */
...
#endif /* WLPFN */
static cmd_func_t wl_wowl_pattern;
...
cmd_t wl_cmds[] = {
	...
	"\torder as npulses, ncontig, min_pw , max_pw, thresh0, thresh1, blank,\n"
	"\tfmdemodcfg, tnpulses_lp, min_pw_lp, max_pw_lp, min_fm_lp,\n"
	...
	"Bit 3 - Wakeup on retrograde tsf (use 'wl wowl_tsf_retr' to configure time)\n"
	"Bit 4 - Wakeup on loss of beacon (use 'wl wowl_bcn_loss' to configure time)\n"},
	{ "wowl_tsf_retr", wl_varint, WLC_GET_VAR, WLC_SET_VAR,
	"Set #of seconds of TSF retrogression for wakeup event "
	"(use 'wl wowl' to configure the event)\n"},
	{ "wowl_bcn_loss", wl_varint, WLC_GET_VAR, WLC_SET_VAR,
	"Set #of seconds of beacon loss for wakeup event (use 'wl wowl' to configure the event)\n"},
	...
	"mask -- Mask to be used for pattern. Bit i of mask corresponds to byte i of the pattern\n"
...
wl_rssi(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (!*++argv) {
		if ((ret = wl_get(wl, cmd->get, &rssi, sizeof(rssi))) < 0)
			return ret;
		printf("%d\n", dtoh32(rssi));
		return 0;
	} else {
		if (!wl_ether_atoe(*argv, &scb_val.ea))
			return -1;
		if ((ret = wl_get(wl, cmd->get, &scb_val, sizeof(scb_val))) < 0)
			return ret;
		printf("%d\n", dtoh32(scb_val.val));
		return 0;
	}
...
wl_version(void *wl, cmd_t *cmd, char **argv)
{
	...
	dump_buf = malloc(SHORT_BUF_LEN);
	if (dump_buf == NULL) {
		fprintf(stderr, "Failed to allocate dump buffer of %d bytes\n", SHORT_BUF_LEN);
		return -1;
	}
	memset(dump_buf, 0, SHORT_BUF_LEN);

	/* query for 'ver' to get version info */
	ret = wl_iovar_get(wl, "ver", dump_buf, SHORT_BUF_LEN);

	if (ret) { /* Error getting version */
		fprintf(stderr, "Version query failed with error %d\n", ret);
		wl_iovar_getint(wl, "bcmerror", &bcmerr);
		fprintf(stderr, "Driver reports error %d\n", bcmerr);
		free(dump_buf);
		return IOCTL_ERROR;
	}
...
wl_channels_in_country(void *wl, cmd_t *cmd, char **argv)
{
	...
	ret = wl_get(wl, cmd->get, buf, sizeof(buf));
	if (ret < 0)
		return ret;
...
wl_join(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (dtoh32(wpa_auth) != -1 &&
	    (ret = wl_set(wl, WLC_SET_WPA_AUTH, &wpa_auth, sizeof(wpa_auth))) < 0)
		return ret;
...
wl_country(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (!stricmp(*argv, "list")) {
		...
		ret = wl_get(wl, WLC_GET_COUNTRY_LIST, buf, sizeof(buf));
		if (ret < 0)
			return ret;
...
wl_radar_args(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (!*++argv) {
		...
		printf("blank 0x%x fmdemodcfg0x%x npulses_lp %d min_pw_lp %d max_pw_lp %d\n",
		       rap->blank, rap->fmdemodcfg, rap->npulses_lp,
		       rap->min_pw_lp, rap->max_pw_lp);
...
wl_antsel(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (!*argv) {
		...
		for (i = UNICAST_TXANT_CFG; i < MAX_NUM_ANT_CFG; i++) {
			if (val.ant_config[i] & WL_ANTCFG_AUTO)
				ant_sel = "auto";
			printf("0x%02x %s ",
			       (val.ant_config[i] & ANTSEL_CONFIG_MASK), ant_sel);
		}
		printf("\n");
	} else {
		/* arg count */
...
wl_pkteng(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (strcmp(cmd->name, "pkteng_stop") == 0) {
		pkteng.flags = WL_PKTENG_CONT_TX_STOP;
		...
		if (strcmp(*argv, "tx") == 0) {
			...
			pkteng.flags =
				(pkteng.nframes ? WL_PKTENG_PER_TX_START : WL_PKTENG_CONT_TX_START);
		}
...
wl_pattern_atoh(char *src, char *dst)
{
	...
	for (i = 0; *src != '\0'; i++) {
		...
		strncpy(num, src, 2);
		num[2] = '\0';
		dst[i] = (uint8)strtoul(num, (char **)&num, 16);
		src += 2;
	}
	return i;
}
