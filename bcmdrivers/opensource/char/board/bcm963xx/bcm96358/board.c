    ...
    else if (iRet > 0)
    {
        if (s_ucWlanButtonCnt > iWPSCounts)
        {
            ...
            else
            {
                ...
                {
                /*START: when PIN mode enabled,pbc still works,by ZZQ00195436 at 2012.1.9*/
            #ifdef  SUPPORT_ATHEROSWLAN
            ...
            #endif  
		/*END:when PIN mode enabled,pbc still works,by ZZQ00195436 at 2012.1.9*/
                }
                else
                {
        			WPS_FLAG = 1;
...
void restore_do_tasklet(void* ptr)
{
    ...
    */
    ...
    /* END:   Modified by y67514, 2009/11/29 */
     ...
     /*Begin:Added by luokunling l00192527,2011/10/17  BN:IMSI Inform*/
     kerSysAvailSet("\0",IFC_IMSI_LEN,IMSI_OFFSET);
     /*End:Added by luokunling l00192527,2011/10/17*/
...
#define ENABLE_WLAN 	17
#define DISABLE_WLAN	18
/*end by aizhanlong 00169427 for DTS2011102004063*/
...
/* HUAWEI HGW s48571 2008年1月18日 Hardware Porting add end.*/
        ...
        case BOARD_IOCTL_CHECK_WLAN_STATUS:   
        	...
        	if (copy_from_user((void*)&ctrlParms, (void*)arg, sizeof(ctrlParms)) == 0)
        	{
             	if ( ctrlParms.action == ENABLE_WLAN)
             	{
             	    g_sWlanFlag = 1;
             	}
             	else if (ctrlParms.action == DISABLE_WLAN)
             	{
             	    g_sWlanFlag = 0;
             	}
             	else
