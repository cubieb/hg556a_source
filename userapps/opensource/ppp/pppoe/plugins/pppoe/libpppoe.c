...
int session_connect(struct session *ses)
{
...
#endif
    /* BEGIN: Added by y67514, 2008/11/19   问题单号:AU8D01206:一条PVC,两个PPPoE连接,插拔adsl线, pppoe连接不能获取地址*/
    if ( strlen(oldsession) )
    {
        session_disconnect_pppoe(ses);
        usleep(100000);
    }
    /* END:   Added by y67514, 2008/11/19 */
	...
	if(ses->retransmits>=0){
	    ++ses->retransmits;
            /* BEGIN: Added by y67514, 2008/11/17   PN:GLB:AutoScan*/
            if ( ses->retransmits > 2 )
            {
                if ( autoscan)
                {
                    exit(0);
            ...
            /* END:   Added by y67514, 2008/11/17 */
	    //tv.tv_sec = 6;
	    if (tv.tv_sec > 3)
		tv.tv_sec = 3;
	    tv.tv_usec = 0;
