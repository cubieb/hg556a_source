 ...
 *
 * libipt_dscp_j.c borrowed heavily from libipt_tos_j.c
 *
...
#include <linux/netfilter_ipv4/ipt_dscp_j.h>
...
static void
parse_dscp(const unsigned char *s, struct ipt_dscp_j_info *dinfo)
{
	unsigned int dscp;
...
static void
parse_class(const unsigned char *s, struct ipt_dscp_j_info *dinfo)
{
...
{
	struct ipt_dscp_j_info *dinfo
		= (struct ipt_dscp_j_info *)(*target)->data;
...
print(const struct ipt_ip *ip,
      ...
      int numeric)
{
	const struct ipt_dscp_j_info *dinfo =
		(const struct ipt_dscp_j_info *)target->data;
	printf("DSCP set ");
...
= { NULL,
    "DSCP",
    IPTABLES_VERSION,
    IPT_ALIGN(sizeof(struct ipt_dscp_j_info)),
    IPT_ALIGN(sizeof(struct ipt_dscp_j_info)),
    &help,
    &init,
    &parse,
