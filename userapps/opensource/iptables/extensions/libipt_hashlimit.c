...
static void hashlimit_init(struct ipt_entry_match *m, unsigned int *nfcache)
{
	struct xt_hashlimit_info *r = (struct xt_hashlimit_info *)m->data;

    memset(r, 0, sizeof(struct xt_hashlimit_info));
...
hashlimit_parse(int c, char **argv, int invert, unsigned int *flags,
                const void *entry, unsigned int *nfcache, struct ipt_entry_match **match)
{
...
static void hashlimit_save(const void *ip, const struct ipt_entry_match *match)
{
	struct xt_hashlimit_info *r = 
		(struct xt_hashlimit_info *)match->data;
...
static struct iptables_match hashlimit_match = {
	.next		= NULL,
	.name		= "hashlimit",
	.version	= IPTABLES_VERSION,
	.size		= IPT_ALIGN(sizeof(struct xt_hashlimit_info)),
...
void _init(void)
{
	register_match(&hashlimit_match);
}
