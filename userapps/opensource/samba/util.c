...
****************************************************************************/
/* HUAWEI HGW w69233 2008-04-07 Modify begin */
/* Modify: Samba Server can start up without /etc/hosts file, */
/* because /etc/hosts could cause confict between HSPA and ADSL process of VDF, which call gethostbyname() Function */
#if 0
...
{
 char *name2 = strdup(name);
 static struct  in_addr inbuf;
 static struct hostent h;
 static char hostname[] = "localhost";
 static char alias_list[2][64] ; 
 struct in_addr *in = &inbuf;
 struct in_addr *addr_list[2];

  if (!name2)
  {
      printf(0,("Memory allocation error in Get_Hostbyname! panic\n"));
      return(NULL);
   }

  /* try with all lowercase */
  strlower(name2);
  if(strncmp(name2,LOCALHOSTNAME,sizeof(LOCALHOSTNAME)) == 0)
  {
      memset(&h, 0, sizeof(struct hostent));
      memset(alias_list, 0,sizeof(alias_list));
      memcpy(alias_list[0], LOCALHOSTNAME, sizeof(LOCALHOSTNAME)); 

     if (inet_pton(AF_INET, LOCALADDR, &inbuf) > 0) {
     	printf("\nFound INET\n");
     	addr_list[0] = in;
     	addr_list[1] = 0;                
     	h.h_name = alias_list[0];
     	h.h_addrtype = AF_INET;
     	h.h_length = sizeof(*in);
	/* A zero-terminated array of network addresses for the host*/	
     	h.h_addr_list = (char**) addr_list; 
	/*A NULL-terminated array of alternate names for the host.*/
     	h.h_aliases = alias_list[1];
...
#endif
/* HUAWEI HGW w69233 2008-04-07 Modify end */
