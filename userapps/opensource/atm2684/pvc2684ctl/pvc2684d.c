...
#include "atmdev.h"      /* ATM device ioctls          */   
...
/* start of PROTOCOL PTM by zhouyi 00037589 2006年7月11日 */
// brcm
setIndexName(char * str, const char * hdr, int index,int vlanId) {  //PTM zyi
    int num1, num2;
    ...
    #ifdef SUPPORT_VDSL
    if ((openTrafficmode == TM_PTM_VLAN) || (openTrafficmode == TM_PTM_PRTCL) || (openTrafficmode == TM_PTM_BR))
    {
    printf("setIndexName1\r\n");
        sprintf(str, "%s_%d_%d_%d", hdr, num1, num2,vlanId);   //ptm zyi
    }
    else
    {
    printf("setIndexName2\r\n");
        sprintf(str, "%s_%d_%d", hdr, num1, num2);   //ptm zyi
    }
    #else
    printf("setIndexName3\r\n");
        sprintf(str, "%s_%d_%d", hdr, num1, num2);   //ptm zyi
    #endif
}
/* end of PROTOCOL PTM by zhouyi 00037589 2006年7月11日 */
...
// brcm
/* start of PROTOCOL PTM by zhouyi 00037589 2006年7月11日 */
int create_br(int nas_idx, int mode,unsigned short vlanId)
/* end of PROTOCOL PTM by zhouyi 00037589 2006年7月11日 */
//int create_br(int nas_idx)
{
  ...
  if((sock = socket(PF_ATMPVC, SOCK_DGRAM, ATM_AAL5)) < 0)
	...
	// brcm
        /* start of PROTOCOL PTM by zhouyi 00037589 2006年7月11日 */
        setIndexName(ni.ifname, "nas", nas_idx,vlanId);
	    ni.id = vlanId;
        /* end of PROTOCOL PTM by zhouyi 00037589 2006年7月11日 */
	...
	// brcm
        /* start of PROTOCOL PTM by zhouyi 00037589 2006年7月11日 */
        setIndexName(ni_rt.ifname, "ipa", nas_idx,vlanId);
	    ni.id = vlanId;
        /* end of PROTOCOL PTM by zhouyi 00037589 2006年7月11日 */
...
//int assign_vcc(struct sockaddr_atmpvc addr, int nas_idx, int encap, int bufsize)
{
  ...
  if ((openTrafficmode == TM_PTM_VLAN) || (openTrafficmode == TM_PTM_PRTCL) || (openTrafficmode == TM_PTM_BR))
  {
      ...
      else 
      {
           ...
           if(proto_filter == 1) // PTM PROTOCOL PPPOE
           {
        	   unId = PROTO_PPPOE;
           }
           else 
           {
               if ((openTrafficmode == TM_PTM_BR)) // PTM PROTOCOL Bridge
    	       {
    	           unId = PROTO_BRIDGE;
    	       }
    	       else // PTM PROTOCOL IPOE
    	       {
    	           unId = PROTO_MER;
    	       }
           }
  ...
  {
      unId = 0; //in atm mode unId not to be used in setIndexName function
  }
  #else
      unId = 0;
  #endif
  
  setIndexName(be.ifspec.spec.ifname, "nas", nas_idx,unId);
  /* end of PROTOCOL PTM by zhouyi 00037589 2006年7月11日 */
  ...
  // brcm
    ...
    // brcm
    /* start of PROTOCOL PTM by zhouyi 00037589 2006年7月11日 */
    setIndexName(be_rt.ifspec.spec.ifname, "ipa", nas_idx,vlan_id);
    /* end of PROTOCOL PTM by zhouyi 00037589 2006年7月11日 */
...
// brcm
 ...
 if ((openTrafficmode == TM_PTM_VLAN) || (openTrafficmode == TM_PTM_PRTCL) || (openTrafficmode == TM_PTM_BR))
 {
     if(rmsg->vlan_id != 0xffff)
     {
           
          ret = create_br(rmsg->nas_idx, rmsg->mode,rmsg->vlan_id);
     }
     else 
     {
    	if(rmsg->proto_filter == 1) // PTM PROTOCOL PPPOE
    	{
    		ret = create_br(rmsg->nas_idx, rmsg->mode,PROTO_PPPOE);
    	}
    	else 
    	{
    	    if ((openTrafficmode == TM_PTM_BR)) // PTM PROTOCOL Bridge
    	    {
    	        ret = create_br(rmsg->nas_idx, rmsg->mode,PROTO_BRIDGE);
    	    }
    	    else // PTM PROTOCOL IPOE
    	    {
    		    ret = create_br(rmsg->nas_idx, rmsg->mode,PROTO_MER);
    	    }
    	}
     }
 ...
 {
     if(create_br(rmsg->nas_idx, rmsg->mode,rmsg->vlan_id)==0)
     {
         smsg.msgtype = INTERFACE_FAILED;
         if( send(cli_fd, &smsg, sizeof(smsg), 0) < 0 )
            do_error(LOG_WARNING,"do_add() couldn't send interface creation failure message.");
...
//  int num, err;
  ...
  if (sock<0) {
    ...
    if( num>=0 && num<1234567890){
      ...
      ni.mtu = 1500;
      /* start of PROTOCOL PTM by zhouyi 00037589 2006年7月11日 */
      setIndexName(ni.ifname, "nas", num,0);  //lyc PTM
      /* end of PROTOCOL PTM by zhouyi 00037589 2006年7月11日 */
...
#ifdef SUPPORT_VDSL
    /* start of PROTOCOL PTM by zhouyi 00037589 2006年7月11日 */
    openTrafficmode = rmsg.trafficmode;
    printf("pvc2684d_main:%d\r\n", openTrafficmode);
    /* end of PROTOCOL PTM by zhouyi 00037589 2006年7月11日 */
#endif
