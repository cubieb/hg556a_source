...
int xbind_connect_tr143(struct sockaddr_in *s_addr, const char *ip, const char *dscp )
{
    struct sockaddr_in stBindAddr;
	int s = socket(AF_INET, SOCK_STREAM, 0);
	int tos = 0;
      	 tos = atoi(dscp) * 4;
	  if ( setsockopt( s, SOL_IP, IP_TOS, (char *)&(tos), 4 )!= 0 )
         {
                printf("*** ERROR setting socket dscp options \n");
          }
