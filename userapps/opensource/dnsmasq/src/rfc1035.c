		      ...
		      struct in_addr local_addr, struct in_addr local_netmask, time_t now) 
{
  ...
  char szNameBuf[64];
  char* pszNameTemp = 0;
  unsigned int uNameSize = 0;
  unsigned char* pucAnsBegin = 0;
  /* c40163 begin ----*/
      ...
      if (qclass == C_IN)
	{
	  ...
	  for (flag = F_IPV4; flag; flag = (flag == F_IPV4) ? F_IPV6 : 0)
		...
		continue;
          ...
          /* c40163 begin ------------ */
          pszNameTemp = strchr(name, '.');
          if (pszNameTemp != 0) {
          	uNameSize = (pszNameTemp - name);
          	if (uNameSize > (sizeof(szNameBuf) - sizeof(char))) {
          	  uNameSize = (sizeof(szNameBuf) - sizeof(char));
          	}
          	memcpy(szNameBuf, name, uNameSize);
          	szNameBuf[uNameSize] = '\0';
          }
          else {
            strncpy(szNameBuf, name, (sizeof(szNameBuf) - sizeof(char)));
            szNameBuf[sizeof(szNameBuf) - sizeof(char)] = '\0';
          }
          ...
          if (!filterCheck(name)) {
            ...
            header->ra = 1;
            header->tc = 0;
            header->rcode = (NOERROR | 0x3);
            header->ancount = htons(0);
            header->nscount = htons(0);
            header->arcount = htons(0);
            return (pucAnsBegin - (unsigned char *)header);
            */
            return 0xffffff;
          }
          /* c40163 end ------------ */
	      ...
	      if (qtype == T_A && (addr.addr.addr4.s_addr = inet_addr(szNameBuf)) != (in_addr_t) -1)
		{
		  ans = 1;
		  if (!dryrun)
		    {
		      log_query(F_FORWARD | F_CONFIG | F_IPV4, szNameBuf, &addr, 0, NULL, 0);
		      if (add_resource_record(header, limit, &trunc, nameoffset, &ansp, 
	    ...
	    cname_restart:
		 /*Start -- w00135358 add for HG556a DNS CACHE*/
		 memset(szNameBuf, 0, 64);
		 /*Start -- a00169427 modify for enable DNS proxy cause DNS dead*/
		 //strcpy(szNameBuf,name);
		 int namelen = strlen(name);
		 strncpy(szNameBuf, name, (namelen < 63 ? namelen : 63));
		 /*End -- a00169427 modify for enable DNS proxy cause DNS dead*/
		 /*End -- w00135358 add for HG556a DNS CACHE*/
	      if ((crecp = cache_find_by_name(NULL, szNameBuf, now, flag | F_CNAME)))
		{
		  int localise = 0;
		  ...
		  if (local_addr.s_addr != 0 && (daemon->options & OPT_LOCALISE) && flag == F_IPV4)
			...
			} while ((crecp = cache_find_by_name(crecp, szNameBuf, now, flag | F_CNAME)));
		      crecp = save;
		    }
		  ...
		  do
		      ...
		      if (crecp->flags & F_CNAME)
			{
			  if (!dryrun)
			    {
			      log_query(crecp->flags, szNameBuf, NULL, 0, daemon->addn_hosts, crecp->uid);
		      ...
		      if (crecp->flags & F_NEG)
			{
			  ...
			  if (crecp->flags & F_NXDOMAIN)
			    nxdomain = 1;
			  if (!dryrun)
			    log_query(crecp->flags, szNameBuf, NULL, 0, NULL, 0);
			}
		      else if ((crecp->flags & (F_HOSTS | F_DHCP)) || !sec_reqd)
			{
			  ...
			  if (!dryrun)
				...
				ttl = crecp->ttd - now;
			      ...
			      log_query(crecp->flags & ~F_REVERSE, szNameBuf, &crecp->addr.addr,
					0, daemon->addn_hosts, crecp->uid);
