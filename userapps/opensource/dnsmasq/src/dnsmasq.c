...
int main (int argc, char **argv)
{
  ...
  /* c40163 begin ----*/
  if (!filterInitialize("/var/url_filter")) {
  	return 0;
  }
  /* c40163 end ----*/
  ...
  while (1)
	...
	if (g_need_dns_response_flag) 
	{
	     ...
	     */
	     /*modify for dns等待超时，导致网关重新双发dns请求*/
		/*Start -- a00169427 modify for enable DNS proxy cause DNS dead*/
		//if (g_get_response_timeout_count++ > 50)
		if (g_get_response_timeout_count++ > 10)
      ...
      if (FD_ISSET(piperead, &rset))
	{
	  ...
	  if (read(piperead, &sig, 1) == 1)
		...
		break;
            ...
            case SIGUSR2:
                g_debug = !g_debug;
                printf("==DNS:%s:%s:%d:g_debug is %d==\n",__FILE__,__FUNCTION__,__LINE__,g_debug);
